...
- **Axiom 3.6 (Implementation Honesty):** only the *sourced* parts land (8+3 wheels, Catalan labels). The 45-pair Tabula Generalis is **deferred** as a surfaced reconstruction — not fabricated as doctrine. The ⚠️ reconstructed sizes (E=22, F=6, S=3) are documented concretizations, flagged in the schema description + docs, not silent.

**Out of scope (explicit, honest):** the letter-level 45-pair Tabula Generalis table (deferred follow-up); a richer `labels` shape (`string[][]`/structured) for the P/T/V/Q/E wheels; a scheduler CLI / W&B tracking.

**State after update:**

- Active master phase remains **Phase 1a**
- **TTCL layer model (working 9-layer axis):** Layers 1-5 + 8 (parity/CI) complete; Layer 6 *core optimizer* landed (PR #38) + **real Llull register + Catalan labels landed (this PR #46)**; Layer 7 *data-generation consumer* landed (PR #39, Catalan labels now sourced) + *training-pipeline wiring* landed (PR #40); the full Layer 7 **GPU run** + human-judged preference pairs + the letter-level 45-pair Tabula Generalis table remain the unbuilt frontier; Layer 9 (live activation) locally ~82-85% complete, capital-gated. Status table in `docs/PROJECT_STATE.md`.
