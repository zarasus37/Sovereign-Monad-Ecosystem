...
**Change Type:** Minor — Layer 7 training-pipeline *wiring* landed; the real TRL pipeline (SFT→Reward→PPO→Eval; LLaMA 3.1 8B + QLoRA + HuggingFace TRL) exists as runnable Python code, import-smoke-verified, CPU-verified. **No training executed** — the 8B GPU run is an explicitly-future job (deliberately not stubbed, no fake completion). This closes the prose's named "next code priority" wiring gap; the GPU execution run remains.

**Summary:**

- **Layer 7 training-pipeline wiring (this PR):** new root-level `gnosis-training/` Python package (uv-managed, sibling to `gnostic-engine/`; spec `TTCL_v1_0_BREAKDOWN.md:275-311`). The real TRL pipeline as runnable code — `SFTTrainer` (Stage 1 SFT on Gnosis-event JSONL with 4-bit QLoRA, assistant loss-masking), `RewardTrainer` (Stage 2, `AutoModelForSequenceClassification` scalar head `num_labels=1`, MSE on the continuous 0-1 `total` score; a 5-way per-criterion head is a documented future refinement behind `RewardConfig.per_criterion_heads=False`), `PPOTrainer` (Stage 3, KL/clip/rollout configs), and an Eval battery (Stage 4, CPU-pure score-regression MAE/RMSE/R² per criterion + total, plus deterministic theo/techno/cosmo coherence proxies). Every spec silence is concretized in `generated/hyperparams.py` (LLaMA 3.1 8B base, LoRA r=64/α=128/dropout=0.05, target_modules, AdamW lr=2e-4 cosine warmup 3%, 4-bit nf4 double-quant, PPO KL β=0.05 / clip 0.2 / rollout batch 128, seed=42, eval MAE/R² thresholds).
- **TRL version pin (load-bearing — do NOT regress):** `trl>=0.12,<0.14` in `pyproject.toml`. The spec names PPO (line 305), but TRL removed the PPO stack (`PPOConfig`/`PPOTrainer`) after the 0.12–0.13 PPOv2 window — gone by 0.14+ and all 1.x (where only DPO/GRPO/KTO remain). `ppo.py` targets the PPOv2-era constructor signature (`args=`, `processing_class=`, `model=`, `ref_model=`, `reward_model=`, `reward_processing_class=`), so the package pins to that exact window — `uv sync` resolves `trl==0.13.0`, where all four symbols (`SFTTrainer`/`SFTConfig`/`RewardTrainer`/`RewardConfig`/`PPOTrainer`/`PPOConfig`) are genuinely importable. PPO is therefore genuinely runnable (spec-faithful, NOT silently substituted with GRPO/DPO — that would be a spec deviation requiring sign-off). Lifting the pin to `trl>=0.14` or `>=1.0` would silently break PPO at import time — do not do this without migrating the spec.
