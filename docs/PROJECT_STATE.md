...
- **x402-bridge sovereignty remediation — LANDED (PR #45, 2026-07-13)** — see the complete list below; the package is **remediated** (no longer `LEGACY_NON_SOVEREIGN`). The live smoke test remains GREEN (2026-07-10, PR #30). See `docs/LEGACY_COMPONENTS.md` §6.
...
- **x402-bridge sovereignty remediation** ✅ (PR #45, 2026-07-13) — bridge wrapped as a sovereign agent (`X402Agent`): cost-accounting ledger (`DrawdownLedger`/`LedgerEntry`, append-only JSONL, one entry per paid call, `to_signal_event()` emission contract mirroring `@sovereign/types` `SignalEvent`/`RevenueEvent`, bus transport deferred); documented failure/retry envelope (`RetryEnvelope` + `request_with_retry`, bounded exponential backoff mirroring `sovereign-bus`, retries 429/503/timeout, `X402_MAX_CONCURRENT` now actually used, `User-Agent` on the RPC path); sovereign-agent consumer with managed resource (settlement address) + cost flow (ledger) + constraint envelope + control surfaces + kill switch. `LEGACY_NON_SOVEREIGN` cleared. Verified offline by 24/24 tests; funded live smoke re-run by a maintainer when ready.
...
- **Data-driven registry** — the scheduler reads wheels + the pair table from `shared/fixtures/layer6/wheel-registry.json` (validated against `shared/ttcl-specs/wheel-registry-schema.json`), so the real Llull register + Catalan slot labels drop in as a *data change*, not a code change. The PR #38 default fixture used the 5-wheel placeholder state space (A/T/V/X/S) with the combinatorial 10-pair set; **that placeholder was replaced in PR #46** (see the next block).
- **Out of scope (honest):** the data-generation *consumer* (turning a schedule into Gnosis events) is Layer 7 territory; the letter-level 45-pair reconstruction (a secondary interpretation — the source states 36 Third-Figure cameras; Llull's Tabula Generalis is the Fourth Figure) is a deferred follow-up (see PR #46); a scheduler CLI / W&B tracking is not in spec scope.
...
**Layer 6 — Real Llull register + Catalan slot labels (PR #46):** the 5-wheel placeholder fixture was replaced with the sourced register from `theo-techno-cosmo/Wheel/8 wheels and 3 domains.docx` — the **8 generative wheels** (A=Blue Dignities 16, P=Prima Figura 9, E=Elements 22, T=T-Wheel 9 [THEOLOGY+COSMOLOGY], V=Virtues&Vices 13, Q=Questions 17, F=Faculties 6, S=Triangles 3) **+ 3 structurally-identical domain wheels** (Teologia/Kosmologia/Technologia, each 16 segments with the shared Catalan B–R vocabulary `essència/eviternitat/vida/pena/dignitats/glòria/acte/segona intenció/forma/primera intenció/relació/exposició/ordinació/manaments/acció/articles`). 11 wheels total. The wheel-level pair table is the math-honest **C(8,2)=28** generative pairs (domain wheels are facets, excluded from the pair table). The schema gained optional per-wheel `alphabet` (the J-skipping 16-letter B–R sequence, carried explicitly) + `labels` (per-slot Catalan labels); the loader enforces `alphabet.length===size` + single-char slots + `labels.length===size`. The consumer now emits **real Catalan labels** for A + the 3 domain wheels + F + S (`active_slots.*.label` is a real string), `null` for the structured-label wheels (P/T/V/Q/E) pending a richer shape.
- **Spec correction (surfaced, not silent — per `spec-open-to-higher-standard`):** the spec's "PairTable (45 pairs, from a triangular grid image)" is a **secondary interpretation, not a statement of the primary Llull source**. The primary source (`Raymond-Lull-Ars-Magna-Ars-Generalis-Ultima.txt`, Chapter 3) states the **Third Figure has 36 cameras** — the distinct 2-letter pairs of B–K (BC…IK, J skipped, C(9,2)=36), with **no self-pairs**. The "45" is a reconstruction = 36 + 9 self-pairs (BB…KK) = the triangular number T₉, which the source does **not** state. Llull's actual **Tabula Generalis / "The Table"** is the **Fourth Figure** (Chapter 4), built from **3-letter cameras** (BCD, BCE…) — a separate, larger structure, not the 2-letter Third Figure and not 45. The earlier wording that called the 45 "the letter-level Tabula Generalis" was a **misattribution and is withdrawn**. The 45 is letter-level (over the 9 Prima-Figura letters), so it does not fit the registry's wheel-level `pairs` field (which stays at C(8,2)=28) and is **deferred** to a follow-up PR as a letter-level composite-validity filter, pending doctrinal confirmation of the 36+9 reconstruction. Surfaced as a NOTE in `TTCL_v1_0_BREAKDOWN.md` at the "Wheel Assets are Concrete" + "Cardinality" sections. The 45 is NOT the 72 Shem↔Goetia name-pairs of the 144 Names (a conflation — the 144 Names are the *locate* layer, separate from Llull's *generate* layer; the real 144-Names↔scheduler link is the PPS triad score).
- **Out of scope (honest, deferred to a follow-up PR):** the letter-level 45-pair reconstruction (a secondary interpretation — the source states 36 Third-Figure cameras; Llull's Tabula Generalis is the Fourth Figure); a richer `labels` shape (`string[][]` / structured) for the P/T/V/Q/E wheels; a scheduler CLI / W&B tracking.

**Verification (PR #46):** `pnpm --filter @sovereign/scheduler exec tsc --noEmit` green; `pnpm --filter @sovereign/gnosis-training-data exec tsc --noEmit` green; `pnpm test:integration` 222/222 (14 files; the scheduler registry/facade assertions + the gnosis Catalan-label honesty gate rewritten for the 11-wheel / 28-pair / sourced-labels register); the checked-in `shared/fixtures/layer6/canonical-schedule.json` regenerates byte-identical from the default seed (seed 42); L7.8 parity (classifier/tier/scorer) unaffected; no `pnpm-lock.yaml` perturbation; no `@sovereign/types` change.
...
- **Honesty posture** — local + deterministic data-generation only; **does not run training** and does not claim live pipeline wiring (the full SFT→Reward→GRPO→Eval pipeline remains unbuilt — a real GPU/TRL job, deliberately not stubbed). Catalan slot labels are now **sourced from the registry** (PR #46) — real strings for A + the 3 domain wheels + F + S, `null` for the structured-label wheels (P/T/V/Q/E); **never fabricated** (at PR #39 they were emitted as `null` pending the asset, which has since landed). The `assistant` message is empty (the SFT training *target* is produced downstream, not synthesized here); the `constitution_score` is the structural score of the prompt scaffold, not a per-response score.
- **Out of scope (honest):** the full Training Pipeline (SFT→Reward→GRPO→Eval; LLaMA 3.1 8B + QLoRA + TRL); the letter-level 45-pair reconstruction (deferred follow-up — the source states 36 Third-Figure cameras; Llull's Tabula Generalis is the Fourth Figure); a scheduler CLI / W&B tracking.
...
### ✅ Completed: Layer 7 dry-run GPU verification + preference-pair corpus (July 14, 2026)

**Dry-run mode (PR #48) + GPU-VERIFIED GREEN 2026-07-14** (RunPod RTX 3090) — the **first end-to-end GPU execution ever** in the project. The full SFT→Reward→GRPO→Eval chain runs on a cheap cloud GPU behind Qwen2.5-0.5B-Instruct + 4-bit QLoRA + SYNTHETIC preference labels, exercising the REAL stage runners via `dataclasses.replace` (the frozen 8B contract intact). Bare CLI modes (`sft`/`reward`/`grpo`/`eval`) stay no-ops unless `--dry-run`; the `dry-run` mode runs the whole chain in one command (runbook: `docs/gnosis-training/DRY_RUN_RUNBOOK.md`). 5 execution-time bugs surfaced + fixed across PR #50-#54 (SFT/Reward adapter merge, GRPO `train_dataset` + `PeftModel` reward load + stale-adapter cleanup, reward tokenizer `pad_token`, reward MODEL config `pad_token_id`, Eval generate **inputs `BatchEncoding` shape). 85 pytest / ruff+mypy clean.
- **Honest reading (load-bearing — do NOT regress):** the dry run proves the pipeline *executes* + emits real numbers, **NOT that the model learns** — degenerate 1-token GRPO completions, `reward_std=0` no-learning, Eval `mae=0`/`r2=1` is the predicted=targets placeholder. The real 8B run + human-judged pairs (spec line 478) remain capital-gated. `uv.lock` untouched; local-only pytest (no Python in `ci.yml`); `pnpm-lock.yaml` untouched.

**Preference-pair corpus — the spec's biggest silence, concretized + audited:**
- **PR #57** (merged 12b1fff): reverts the #56 overclaim — the 240-pair jsonl was TEMPLATED generator output (constant 0.927, 156 unique chosen / 36 unique rejected across 240, CAT7 = prompt echoed 4×) that passed `validate_pair` because it only checked SCORES, never the response TEXT. Replaced with the 25 REAL exemplars authored inline in `REWARD_MODEL_PREFERENCE_PAIRS_GUIDE.md` (CAT1 8 / CAT2 7 / CAT3 5 / CAT4 5; CAT5–7 + the 200–300 target still pending).
- **PR #58** (merged c23258b): committed the 763-line LOGOC corpus source (only the derived jsonl had been committed) + completed the 3 unscored exemplars (P5 0.71, P15 0.70, P20 0.70) + synced the canonical jsonl.
- **PR #59** (merged 8a64f1f): the `validate_pair` **content check** — RULE 3 (per-pair: non-empty + not a prompt echo), RULES 4/5 (worksheet: response diversity ≥ 0.40 unique/total + non-constant chosen total). A `synthetic: bool` field skips the dry-run stand-ins so the GPU-verified dry run is preserved; real human pairs default `synthetic=False` → guard applies → a future #56 is blocked at both `load_human_pairs` AND `validate-worksheet`. 103 pytest / ruff+mypy clean; the real 25-pair file passes; a simulated #56 is correctly caught.
- **PR #60** (merged cad1355): 15 Claude-drafted CAT5/6/7 candidate pairs (PROMPTs 26–40) appended to the guide as a clearly-demarcated "CANDIDATE PAIRS — CLAUDE-DRAFTED, PENDING USER REVIEW" section. **NOT** promoted into the corpus or the jsonl (the #56 lesson: AI-authored = "Claude-drafted, user-reviewed", provenance must be honest). 5 CAT5 (no-RLHF vs RLHF-contaminated), 5 CAT6 (near-miss, one per failing criterion C1–C5), 5 CAT7 (DeFi/poker/AI/theology/sovereignty). Promotion to the corpus requires human sign-off per pair.
- **PR #62** (this branch — user-authored pairs promoted, July 15, 2026): the human author reviewed the 15 PR #60 candidates and authored their OWN chosen/rejected responses to 13 of them (PROMPTs 26–28, 31–40, skipping 29/30) + 3 new (PROMPTs 41–43) = **16 human-judged pairs**, appended under a LOGOC TRAINING CORPUS header after "END CANDIDATE PAIRS". Categories reviewed against the rejected's actual failure mode (precedence: moralizing→CAT3, false-certainty→CAT4, RLHF→CAT5; else 2-domains-present→CAT1, 1-domain-only→CAT2) — 14 correct as labeled, **2 swapped + fixed**: PROMPT 31 CAT2→CAT1 (rejected has tech+cosmology, missing only theology = TYPE A not B) and PROMPT 43 CAT1→CAT2 (rejected is operational-only, missing theology AND cosmology = TYPE B not A). All 16 approved (3-domain traversal + real logic-compression JOIN in every chosen; clear stated failure in every rejected; genuine gaps 0.22–0.52). Serialized into `preference_pairs_ALL.jsonl` as PP-026..PP-041 (uniform sub-scores = guide Score, failing_criteria A/B→C1, C→C5, D→C4, E→C5, `synthetic: false`) via `gnosis-training/scripts/promote_user_pairs.py` (parses the guide, validates each pair, runs the worksheet guard over all 41, appends — idempotent). **25 → 41 human-judged pairs.** The 15 Claude-drafted candidates are superseded (a SUPERSEDED marker added to their section header; they remain in the guide as provenance only — NOT in the jsonl). `validate-worksheet` → 41 ok / 0 invalid; 103 pytest / ruff+mypy clean. Category spread now CAT1 13 / CAT2 10 / CAT3 8 / CAT4 9 / CAT5 1 / CAT6 0 / CAT7 0 / CAT8 0 (CAT6/7/8 still open; CAT5 has its first human-judged exemplar — the markets/RLHF pair).
...
| 6 | Scheduler (Multi-Objective Opt.) | 🟡 | Core optimizer landed (PR #38): SA over wheel rotations → `canonical_schedule.json`; `J = αC+βL+γT−δS`; three moves by spec probs; seeded PRNG; data-driven registry; byte-reproducible artifact. **Real Llull register landed (PR #46):** 8 generative wheels (A/P/E/T/V/Q/F/S) + 3 domain wheels (Teologia/Kosmologia/Technologia) with the sourced 16-segment Catalan B–R vocabulary — 11 wheels, replacing the 5-wheel placeholder; wheel-level pair table = C(8,2)=28 | Letter-level 45-pair reconstruction (a secondary interpretation — the primary source states 36 Third-Figure cameras with no self-pairs; the "45" = 36+9 self-pairs = T₉ is unsourced; Llull's actual Tabula Generalis is the Fourth Figure, not 45 — a deferred follow-up, surfaced in the spec NOTE, not a wheel-pair count); richer `labels` shape for the structured wheels (P/T/V/Q/E); scheduler CLI / W&B tracking |
| 7 | Training Pipeline (Full Stack) | 🟡 | Data-gen consumer landed (PR #39): `@sovereign/gnosis-training-data` → Gnosis event JSONL, reuses `scoreSign`, deterministic + byte-reproducible; **Catalan slot labels now sourced from the registry (PR #46)** — real strings for A + the 3 domain wheels + F + S, `null` for the structured-label wheels (P/T/V/Q/E); **training-pipeline wiring landed**: `gnosis-training/` — real `SFTTrainer`/`RewardTrainer`/`GRPOTrainer` + QLoRA configs as runnable code, import-smoke-verified, CPU-verified; **dry-run mode landed (PR #48) + GPU-VERIFIED GREEN 2026-07-14** (RunPod RTX 3090 — the first end-to-end GPU execution ever): the full SFT→Reward→GRPO→Eval chain runs on Qwen2.5-0.5B + 4-bit QLoRA + SYNTHETIC preference labels; 5 execution-time bugs surfaced + fixed across PR #50-#54 (SFT/Reward adapter merge, GRPO train_dataset + PeftModel reward load + stale-adapter cleanup, reward tokenizer pad_token, reward MODEL config pad_token_id, Eval generate BatchEncoding shape); **preference-pair corpus**: 25 REAL human-judged exemplars (PR #57, reverting the #56 overclaim where a templated 240-pair file passed as human-judged) + the LOGOC corpus source committed (PR #58) + the `validate_pair` content check RULES 3/4/5 (PR #59, reads response TEXT not just scores — blocks future templated files) + 15 Claude-drafted CAT5/6/7 candidate pairs (PR #60, pending user review) + **16 user-authored/reviewed pairs promoted (PR #62, 25→41 human-judged; the 15 candidates superseded by the user's authored versions)** | Real 8B **GPU run** (LLaMA 3.1 8B + QLoRA + HuggingFace TRL — capital-gated; the dry run proved the pipeline *runs*, not that the model *learns*: degenerate 1-token GRPO completions, reward_std=0 no-learning, Eval mae=0/r2=1 is the predicted=targets placeholder — honest reading); human-judged preference pairs toward the 200–300 target (spec line 478; 41/250 real — CAT5 has 1 exemplar, CAT6/7/8 still open); fix apeiron over-strictness (CAT8 blocked); letter-level 45-pair reconstruction (deferred follow-up — the source states 36 Third-Figure cameras; Llull's Tabula Generalis is the Fourth Figure) |
| 8 | Parity / CI | ✅ | Scorer/tier/classifier parity CI-enforced; 222 integration tests | Keep green as Layer 7 lands |
...
**Throughline:** Layers 1-5 + 8 done. Layer 5 is spec-complete: L3 + L2 (inference/rewrite/fusion/constitution/budget) + L1 Provenance all landed (PR #25/#35/#37); the only remaining Layer 5 extra is the schema drift guard (a CI nicety) — code-emission L0 and Python parity are explicit non-goals by design. Layer 6's *core optimizer* landed in PR #38 (`canonical_schedule.json` is generated + byte-reproducible); the **real Llull register + Catalan slot labels landed in PR #46** (8 generative + 3 domain wheels, sourced Catalan B–R vocabulary, replacing the 5-wheel placeholder; wheel-level pairs = C(8,2)=28; the spec's "45 pairs" is a deferred letter-level reconstruction — the source states 36 Third-Figure cameras; Llull's actual Tabula Generalis is the Fourth Figure — surfaced in the spec NOTE). Layer 7's *data-generation consumer* landed in PR #39 (`@sovereign/gnosis-training-data` → Gnosis event JSONL, reusing `scoreSign`, Catalan labels now sourced); Layer 7's **training-pipeline wiring** landed (`gnosis-training/` — real TRL trainers + QLoRA as runnable code, import-smoke-verified, CPU-verified); **Layer 7's dry-run mode landed (PR #48) and is GPU-VERIFIED GREEN 2026-07-14** (RunPod RTX 3090 — the first end-to-end GPU execution ever; the full SFT→Reward→GRPO→Eval chain runs on Qwen2.5-0.5B + 4-bit QLoRA + SYNTHETIC labels, with 5 execution-time bugs fixed across PR #50-#54). The remaining frontier is the **real 8B GPU run** (LLaMA 3.1 8B + QLoRA + HuggingFace TRL — capital-gated; the dry run proved the pipeline *runs*, not that the model *learns*) + **human-judged preference pairs** toward the 200–300 target (spec line 478; 41 real pairs now — PR #57's 25 + PR #62's 16 user-authored/reviewed; the 15 PR #60 candidates superseded; CAT5 has 1 exemplar, CAT6/7/8 still open) + a `validate_pair` content check (RULES 3/4/5, PR #59) that reads response TEXT not just scores — the structural fix for the #56 overclaim where a templated 240-pair file passed as human-judged + the letter-level 45-pair reconstruction follow-up (and the Fourth-Figure Tabula Generalis). Layer 9 is mostly external/capital.

## Completed: TTC operational constraints v1.0.0 → v1.1.0 (2026-07-16)

Philosophy → enforceable system validity (orthogonal to constitution C1–C5):

- `docs/THEO_TECHNO_COSMO.md` — operational source of truth (v1.1.0)
- `shared/constraints/v1.0.0/` (immutable) + **`v1.1.0/`** (`CURRENT` → **1.1.0**)
- v1.1 features: **adaptive refusal** (25% exploration / 12% stable), **sovereignty debt** (threshold 5 → forced refusal), **weighted composite_score** (0.4/0.3/0.3), **drift amnesty** (5 actions after declared fingerprint change)
- `gnostic-engine/.../constraints/` — loader, refusal window, identity tracker, debt ledger, `score_ttc` / `gate_ttc`
- **Hepar live hard gate:** `@sovereign/hepar-defi-auditor` `runDefiAuditGated` → `gateTtc` **before** any `sovereignBus.emit`; non-throwing outcome via `runDefiAuditGated` / `HeparOrchestrator.runFullAuditGated`
- **Relay defense-in-depth:** `POST /api/v1/hepar/submit` gates before buffer/event log
- Agent design: `docs/AGENT_DESIGN_DECLARATION_TEMPLATE.md` + `docs/agents/HEPAR_DESIGN_DECLARATION.md`
- Tests: `gnostic-engine/tests/test_ttc_constraints.py` (v1.1)

### Completed: CAT9 TTC preference pairs + multi-obj hooks (2026-07-18)

- `docs/gnosis-training/TTC_PREFERENCE_PAIRS_GUIDE.md` — CAT9 (+30 additive): 10 T / 10 X / 10 C; 12 candidate drafts **NOT in jsonl** pending human authoring
- Pointer in `REWARD_MODEL_PREFERENCE_PAIRS_GUIDE.md` (CAT 9 section)
- `gnosis_training/ttc_signals.py` — composite weights, multi-obj blend (0.55 constitution + 0.45 TTC), lexical proxies, `TtcWindowMetrics`
- Preference schema: optional `ttc_axis`, `chosen_ttc`, `rejected_ttc` + **RULE T1** composite gap ≥ 0.10
- `reward.pairs_to_multiobjective_rows` for margin / future multi-head training
...
- Wired into `TtcConstraintScorer.score` (pass **and** fail) before any bus emit path
- Python: `gnostic_engine.constraints.window_log` — same schema on every `gate_ttc` / scorer record (incl. `/api/v1/hepar/submit`)
- Logs: `logs/ttc-window/hepar-defi-auditor.jsonl`, `logs/ttc-window/gnostic-engine.jsonl` (gitignored `*.jsonl`)
- CLI: `python -m gnosis_training ttc-window-report`

**Still open:** human-author + promote CAT9 candidates into `preference_pairs_ALL.jsonl` (0/30); multi-head RewardTrainer consuming multi-obj targets; Cardia/Data Rail hard gates; UI dashboard on top of JSONL.

### Active pivot: Vector 1 — Shaliah Onboarding Arc (2026-07-20)

- **Design:** `docs/SHALIAH_ONBOARDING_ARC.md` (UMS three-phase stealth education; closes SHALIAH_AGENTS §6.2 open task designs)
- **Package:** `@sovereign/shaliah-onboarding` — pure TS phase machines + 9/9 tests + `pnpm demo`
  - Phase 1 `phase1Circuit` — Llull T/X/C board; overload / starve; `CognitiveTwinSeed`
  - Phase 2 `phase2ShadowMarket` — shadow MEV deck; spurious red vs correct green halt; name refusal
  - Phase 3 `phase3Archon` — free-text fail; structured envelope + audit_trace + rule pass
  - `arc.ts` state machine → `graduated` (Meshaleach token local-only)
- **Control Center UI landed:** `/onboarding/broken-genesis` · `/onboarding/shadow-market` · `/onboarding/archon-gate` + local PL (+10/+15/+25)
- **Not yet:** JSONL under `logs/onboarding/` + hcd-monitor parsers; production Kafka PL; mint dormant NFT / live capital unlock
- **Hermes note:** Layer-6 scheduler / letter-pairs work is separate — do not mix branches without handoff

### ✅ Vector 6.1 — Stage 2 RewardTrainer lock (`gnosis-v2.0-reward`) — 2026-07-20

- **Pre-train audit:** `docs/GNOSIS_V2_PRETRAIN_AUDIT.md` — honest correction: task claimed 71 pairs in `preference_pairs_worksheet.jsonl`; on disk that file is **250 bootstrap/empty** rows. Trainable corpus is `preference_pairs_ALL.jsonl` — **41** human-judged pairs (CAT1–5; **no CAT6–9 yet**).
- **CLI:** `python -m gnosis_training reward data/preference_pairs_ALL.jsonl --stage2-v2` (`stage2_v2.py`)
- **Checkpoint:** `gnosis-training/checkpoints/gnosis-v2.0-reward/` + `MODEL_CARD.md`
- **This machine:** **CPU_VERIFIED_DRY_RUN** (no CUDA) — Qwen2.5-0.5B-Instruct, lr=1e-5, max_steps=4, 80/20 split (32/9), train_loss≈0.46, eval_loss≈0.365, eval_accuracy=0.8. Proves tokenizer + RewardTrainer + loss path on **human** pairs — **not** a full 3-epoch 8B run.
- **Training log:** `docs/GNOSIS_V2_TRAINING_LOG.md`
- **Still open for production Stage 2:** promote CAT6–9 (incl. CAT9 TTC), grow toward 200–300 pairs, GPU 8B QLoRA full epochs.

## Next Actions

1. **Onboarding → Live path** — V1–V4.4 + **V5.2–5.3** (`@sovereign/monad-mev`: capacity ceiling + EIP-712 CapitalMandate + Router B 50/40/10 yield). Remaining: production Hepar URL + key custody; real swap calldata; onboarding JSONL → hcd-monitor.
2. **Author CAT9 pairs** — review TTC BATCH 1 candidates in `docs/gnosis-training/TTC_PREFERENCE_PAIRS_GUIDE.md`, write full tripartite chosen/rejected text, promote into `preference_pairs_ALL.jsonl`, run `validate-worksheet` + `ttc-metrics` (target 30; separate from onboarding arc). Grow corpus past 41 toward 200–300.
3. **Code frontier — Layer 7** (**Stage 2 v2.0 first-breath lock COMPLETE** as CPU verified dry-run on 41 human pairs; dry-run GPU-verified on synth; real 8B full-epoch run capital-gated): the Layer 6 Scheduler *core optimizer* landed in PR #38 (`canonical_schedule.json` generated + byte-reproducible), the Layer 7 **data-generation consumer** landed in PR #39 (`@sovereign/gnosis-training-data` → Gnosis event JSONL, reusing `scoreSign`, deterministic + byte-reproducible), the Layer 7 **training-pipeline wiring** landed (`gnosis-training/` — real `SFTTrainer`/`RewardTrainer`/`GRPOTrainer` + QLoRA configs as runnable code, import-smoke-verified, CPU-verified), and the **Layer 7 dry-run mode landed (PR #48) and is GPU-VERIFIED GREEN 2026-07-14** (RunPod RTX 3090 — the first end-to-end GPU execution ever; the full SFT→Reward→GRPO→Eval chain runs on Qwen2.5-0.5B + 4-bit QLoRA + SYNTHETIC labels, with 5 execution-time bugs fixed across PR #50-#54). The remaining frontier is the **real 8B GPU run** — SFT→Reward→GRPO→Eval; LLaMA 3.1 8B + QLoRA + HuggingFace TRL — capital-gated. **Honest reading of the dry run:** it proved the pipeline *runs* (real gradients, real eval numbers), NOT that the model *learns* — degenerate 1-token GRPO completions, `reward_std=0` no-learning, Eval `mae=0`/`r2=1` is the predicted=targets placeholder. The reward-model training signal is **human-judged preference pairs** (spec line 478): 25 real exemplars are authored (PR #57, reverting the #56 overclaim where a templated 240-pair file passed as human-judged) + the LOGOC corpus source is committed (PR #58) + a `validate_pair` content check (RULES 3/4/5, PR #59) reads response TEXT not just scores (the structural fix so a future templated file cannot pass) + 15 Claude-drafted CAT5/6/7 candidate pairs were drafted (PR #60) and **the user reviewed them and authored 16 of their own (PR #62, 25→41 human-judged; the 15 candidates superseded)**. Toward the 200–300 target, the open authoring work is: author more pairs (CAT6/7 + CAT8 once apeiron over-strictness is fixed; CAT5 has its first exemplar), and add a `validate_pair` CONTENT check so templated files can't pass — the latter landed in PR #59. A minor Layer 5 extra — a schema drift guard CI check for `semiotic-program-schema.json` — can land alongside.
4. **Layer 6 follow-ups (asset / data, not code)** — the real Llull register + Catalan slot labels + the `Theologia` (= Teologia) wheel **landed in PR #46** (8 generative + 3 domain wheels, sourced Catalan B–R vocabulary). The remaining Layer 6 follow-up is the **letter-level 45-pair reconstruction** (a secondary interpretation — the primary source states 36 Third-Figure cameras with no self-pairs; the "45" = 36+9 self-pairs = T₉ is unsourced, and Llull's actual Tabula Generalis is the Fourth Figure, not 45 — a letter-level composite-validity filter, not the wheel-level pair table which stays at C(8,2)=28; deferred pending doctrinal confirmation) + a richer `labels` shape for the structured wheels (P/T/V/Q/E). A scheduler CLI / W&B tracking is out of spec scope.
5. **x402-bridge sovereignty remediation** — **landed (PR #45)**: the bridge is wrapped as a sovereign agent (`X402Agent`) — cost-accounting ledger (`DrawdownLedger`/`LedgerEntry`), documented failure/retry envelope (`RetryEnvelope` + `request_with_retry`, mirrors `sovereign-bus` backoff; `X402_MAX_CONCURRENT` now used; `User-Agent` on the RPC path), and a sovereign-agent consumer with documented control surfaces + kill switch. The `LEGACY_NON_SOVEREIGN` flag is cleared in `docs/LEGACY_COMPONENTS.md` §6. Verified offline by 24/24 tests; the funded live smoke (green 2026-07-10, PR #30) is re-run by a maintainer when ready. Capital-gated live frontiers that still stand: funded Cardia, live Keys, public Data Rail.
6. Continue normal development inside the approved active domains only.
7. Update this file whenever the project state changes in a meaningful way.
8. Run `pnpm check:layout` after structural changes (it now degrades gracefully when `ripgrep` is unavailable).
