...
1. **Onboarding → Live path** — V1 phases + V3 PL bridge + V3.2 wallet bind + **V3.3 Cardia Funding Engine**: Hepar-gated `executeFunding`; dry-run `TX_SYNTHESIZED` by default; live ERC-20 when `CARDIA_FUNDING_LIVE` + RPC/key/stablecoin; consumer fires only on `wallet-bind-tier1-activation`. Remaining: mount HTTP host; production Bootstrap key custody; Control Center funding status UI; onboarding JSONL → hcd-monitor.
