...
1. **Onboarding → Live path** — Vector 1 phases + **V3 PL bridge** + **V3.2 wallet bind**: `bindWallet` / `bindWalletHttp` (EIP-191 + server PL ≥ 50); UI `/onboarding/live-activation`; Cardia unlock requires `taskId: wallet-bind-tier1-activation` (not simulation PL alone). Remaining: mount HTTP on host; Cardia Funding Engine (Node 2); onboarding JSONL → hcd-monitor.
