...
1. **Vector 1 Onboarding Arc — complete (UI + local PL)** — Phase 1–3 routes + Meshaleach. **Vector 3 Kafka PL bridge landed:** `promotePl` / `promotePlHttp` in `@sovereign/gate-acl` (server verify → `pl.events` + `sovereign.pl.ledger.events`); frontend `promotePlToBridge` + `markSynced`; Cardia `evaluatePlForTier1` / `startPlLedgerConsumer` (Tier-1 = PL ≥ 50 → $15k mandate eligible). Set `KAFKA_ENABLED=true` for live emit. Remaining: wire HTTP route in Azure/Express host; funded capital execution; onboarding JSONL → hcd-monitor.
2. **Author CAT9 pairs** — review TTC BATCH 1 candidates in `docs/gnosis-training/TTC_PREFERENCE_PAIRS_GUIDE.md`, write full tripartite chosen/rejected text, promote into `preference_pairs_ALL.jsonl`, run `validate-worksheet` + `ttc-metrics` (target 30; separate from onboarding arc).
