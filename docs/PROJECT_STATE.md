...
- **Out of scope (honest):** the data-generation *consumer* (turning a schedule into Gnosis events) is Layer 7 territory; the letter-level 45-pair Tabula Generalis table is a deferred follow-up (see PR #46); a scheduler CLI / W&B tracking is not in spec scope.
...
- **Spec correction (surfaced, not silent — per `spec-open-to-higher-standard`):** the spec's "PairTable (45 pairs, from a triangular grid image)" is the **letter-level Tabula Generalis** — the 36 Third-Figure cameras C(9,2) **plus the 9 self-pairs** (BB…KK) = the triangular number T₉ = 45 — over the 9 Prima-Figura letters (B–K), *not* a wheel-level pair count. It does not fit the registry's wheel-level `pairs` field (which stays at 28) and is **deferred** to a follow-up PR as a letter-level composite-validity filter, pending doctrinal confirmation of the 36+9 reconstruction. Surfaced as a NOTE in `TTCL_v1_0_BREAKDOWN.md` at the "Wheel Assets are Concrete" + "Cardinality" sections. The 45 is NOT the 72 Shem↔Goetia name-pairs of the 144 Names (a conflation — the 144 Names are the *locate* layer, separate from Llull's *generate* layer; the real 144-Names↔scheduler link is the PPS triad score).
- **Out of scope (honest, deferred to a follow-up PR):** the letter-level 45-pair Tabula Generalis table; a richer `labels` shape (`string[][]` / structured) for the P/T/V/Q/E wheels; a scheduler CLI / W&B tracking.
...
- **Out of scope (honest):** the full Training Pipeline (SFT→Reward→GRPO→Eval; LLaMA 3.1 8B + QLoRA + TRL); the letter-level 45-pair Tabula Generalis table (deferred follow-up); a scheduler CLI / W&B tracking.
