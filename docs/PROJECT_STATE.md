...
| 6 | Scheduler (Multi-Objective Opt.) | 🟡 | Core optimizer landed (PR #38): SA over wheel rotations → `canonical_schedule.json`; `J = αC+βL+γT−δS`; three moves by spec probs; seeded PRNG; data-driven registry; byte-reproducible artifact | Data-generation *consumer* (Layer 7); real 45-pair Llull PairTable + Catalan slot labels + `Theologia` wheel (asset reconciliation); scheduler CLI / W&B tracking |
| 7 | Training Pipeline (Full Stack) | ⬜ | Parity prerequisites only | SFT→Reward→PPO→Eval; LLaMA 3.1 8B + QLoRA + TRL; the data-gen consumer of the Layer 6 schedule |
| 8 | Parity / CI | ✅ | Scorer/tier/classifier parity CI-enforced; 209 integration tests | Keep green as Layers 6/7 land |
...
**Throughline:** Layers 1-5 + 8 done. Layer 5 is spec-complete: L3 + L2 (inference/rewrite/fusion/constitution/budget) + L1 Provenance all landed (PR #25/#35/#37); the only remaining Layer 5 extra is the schema drift guard (a CI nicety) — code-emission L0 and Python parity are explicit non-goals by design. Layer 6's *core optimizer* landed in PR #38 (`canonical_schedule.json` is generated + byte-reproducible); the data-generation consumer + the real 45-pair Llull PairTable are Layer 7 / asset follow-ups. Layer 7 (Training Pipeline) is now the unbuilt core and the active code frontier. Layer 9 is mostly external/capital.

## Next Actions

1. **Code frontier — Layer 7** (`@sovereign/scheduler` consumer): the Layer 6 Scheduler *core optimizer* landed in PR #38 (`canonical_schedule.json` generated + byte-reproducible). The unbuilt core is now Layer 7 — the Training Pipeline (SFT→Reward→PPO→Eval; LLaMA 3.1 8B + QLoRA + TRL) *and* the data-generation consumer that turns a Layer 6 schedule into Gnosis training events. A minor Layer 5 extra — a schema drift guard CI check for `semiotic-program-schema.json` — can land alongside either.
