...
| 7 | Training Pipeline (Full Stack) | 🟡 | Data-gen consumer landed (this PR): `@sovereign/gnosis-training-data` → Gnosis event JSONL, reuses `scoreSign`, deterministic + byte-reproducible, labels `null` pending the asset | Full SFT→Reward→PPO→Eval pipeline; LLaMA 3.1 8B + QLoRA + HuggingFace TRL (real GPU job, not stubbed); 45-pair PairTable + Catalan labels + `Theologia` wheel (data asset) |
...
**Throughline:** Layers 1-5 + 8 done. Layer 5 is spec-complete: L3 + L2 (inference/rewrite/fusion/constitution/budget) + L1 Provenance all landed (PR #25/#35/#37); the only remaining Layer 5 extra is the schema drift guard (a CI nicety) — code-emission L0 and Python parity are explicit non-goals by design. Layer 6's *core optimizer* landed in PR #38 (`canonical_schedule.json` is generated + byte-reproducible). Layer 7's *data-generation consumer* landed in this PR (`@sovereign/gnosis-training-data` → Gnosis event JSONL, reusing `scoreSign`); the full SFT→Reward→PPO→Eval pipeline + the real 45-pair Llull PairTable remain the unbuilt core / asset follow-ups. Layer 7's full Training Pipeline is now the unbuilt core and the active code frontier. Layer 9 is mostly external/capital.

## Next Actions

1. **Code frontier — Layer 7** (full Training Pipeline): the Layer 6 Scheduler *core optimizer* landed in PR #38 (`canonical_schedule.json` generated + byte-reproducible), and the Layer 7 **data-generation consumer** landed in this PR (`@sovereign/gnosis-training-data` → Gnosis event JSONL, reusing `scoreSign`, deterministic + byte-reproducible). The unbuilt core is now the **full Layer 7 Training Pipeline** — SFT→Reward→PPO→Eval; LLaMA 3.1 8B + QLoRA + HuggingFace TRL — a real GPU job (not to be stubbed). A minor Layer 5 extra — a schema drift guard CI check for `semiotic-program-schema.json` — can land alongside.
