...
- **x402-bridge sovereignty remediation — LANDED (PR #<n>, 2026-07-13)** — see the complete list below; the package is **remediated** (no longer `LEGACY_NON_SOVEREIGN`). The live smoke test remains GREEN (2026-07-10, PR #30). See `docs/LEGACY_COMPONENTS.md` §6.
...
- **x402-bridge sovereignty remediation** ✅ (PR #<n>, 2026-07-13) — bridge wrapped as a sovereign agent (`X402Agent`): cost-accounting ledger (`DrawdownLedger`/`LedgerEntry`, append-only JSONL, one entry per paid call, `to_signal_event()` emission contract mirroring `@sovereign/types` `SignalEvent`/`RevenueEvent`, bus transport deferred); documented failure/retry envelope (`RetryEnvelope` + `request_with_retry`, bounded exponential backoff mirroring `sovereign-bus`, retries 429/503/timeout, `X402_MAX_CONCURRENT` now actually used, `User-Agent` on the RPC path); sovereign-agent consumer with managed resource (settlement address) + cost flow (ledger) + constraint envelope + control surfaces + kill switch. `LEGACY_NON_SOVEREIGN` cleared. Verified offline by 24/24 tests; funded live smoke re-run by a maintainer when ready.
...
3. **x402-bridge sovereignty remediation** — **landed (PR #<n>)**: the bridge is wrapped as a sovereign agent (`X402Agent`) — cost-accounting ledger (`DrawdownLedger`/`LedgerEntry`), documented failure/retry envelope (`RetryEnvelope` + `request_with_retry`, mirrors `sovereign-bus` backoff; `X402_MAX_CONCURRENT` now used; `User-Agent` on the RPC path), and a sovereign-agent consumer with documented control surfaces + kill switch. The `LEGACY_NON_SOVEREIGN` flag is cleared in `docs/LEGACY_COMPONENTS.md` §6. Verified offline by 24/24 tests; the funded live smoke (green 2026-07-10, PR #30) is re-run by a maintainer when ready. Capital-gated live frontiers that still stand: funded Cardia, live Keys, public Data Rail.
