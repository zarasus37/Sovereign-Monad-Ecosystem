...
if st.sidebar.button("Run Diagnostic"):
    payload: dict[str, object] = {"var_id": var_id, "lane_a": l_a, "lane_b": l_b, "lane_c": l_c}
    try:
        res = requests.post("http://127.0.0.1:8000/intake/forage", json=payload).json()
        
        col1, col2, col3 = st.columns(3)
        col1.metric("Verdict", res['verdict'])
        col2.metric("Structural Read", res['structural_read'])
        col3.metric("Phase Tilt", f"{res['phase_tilt']}°")
