...
### Active frontier
- **Code — Layer 7 (the unbuilt core):** the Layer 6 Scheduler *core optimizer* landed in PR #38 (`canonical_schedule.json` is generated + byte-reproducible), and the Layer 7 **data-generation consumer** landed in this PR (`@sovereign/gnosis-training-data` → Gnosis event JSONL, reusing `scoreSign`). The unbuilt core is now the **full Layer 7 Training Pipeline** (SFT→Reward→PPO→Eval; LLaMA 3.1 8B + QLoRA + TRL — a real GPU job, not stubbed). The real 45-pair Llull PairTable + Catalan slot labels + `Theologia` wheel remain a data-asset follow-up (drop-in, no code change).
