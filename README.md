...
| **Gnosis Training** | `gnosis-training/` | TTCL Layer 7 Training Pipeline (SFT→Reward→GRPO→Eval); real TRL wiring, CPU-verified, GPU run pending | Python (uv) |
...
- **Layer 7 training-pipeline wiring — landed** (`gnosis-training/`, new root-level Python package): real `SFTTrainer`/`RewardTrainer`/`GRPOTrainer` + QLoRA configs as runnable code, import-smoke-verified. **No training executed** — the LLaMA 3.1 8B run is an explicitly-future GPU job. A preference-pair builder emits a bootstrap worksheet (the reward model still trains on human-judged pairs per spec line 478). W&B/MLflow/DVC wired-but-deferred behind opt-in extras. CPU-verified; GPU run pending.
...
### Active frontier
- **Code — Layer 7 (the wiring landed, GPU run pending):** the Layer 6 Scheduler *core optimizer* landed in PR #38 (`canonical_schedule.json` generated + byte-reproducible), the **real Llull register + Catalan labels landed in PR #46** (8 generative + 3 domain wheels, sourced 16-segment Catalan B–R vocabulary, replacing the 5-wheel placeholder; the wheel-level pair table is C(8,2)=28; the spec's "45 pairs" is a deferred letter-level reconstruction — not the primary Llull source, see the spec NOTE), the Layer 7 **data-generation consumer** landed in PR #39 (`@sovereign/gnosis-training-data` → Gnosis event JSONL, reusing `scoreSign`, Catalan labels now sourced), and the Layer 7 **training-pipeline wiring** landed (`gnosis-training/` — real `SFTTrainer`/`RewardTrainer`/`GRPOTrainer` + QLoRA configs, import-smoke-verified, CPU-verified). The remaining frontier is the **full Layer 7 GPU run** (SFT→Reward→GRPO→Eval; LLaMA 3.1 8B + QLoRA + HuggingFace TRL — a real GPU job, not stubbed, not yet executed). Human-judged preference pairs (spec line 478) are the reward-model training signal. The 45-letter-pair reconstruction and Llull's Fourth-Figure Tabula Generalis remain deferred follow-ups.
